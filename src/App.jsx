    ...
    <div className="flex">
      ...
      <header className="md:hidden fixed bottom-0 left-0 w-full bg-white bg-opacity-90 backdrop-blur-md flex justify-around items-center py-2 shadow-t-lg z-50">
        {[Home, Star, ListChecks, Car, MessageCircle].map((Icon, i) => {
          ...
          return (
            <button key={s.id} onClick={() => scrollTo(s.id)} className="flex flex-col items-center text-xs">
              <Icon size={20} className={isActive ? "text-green-700" : "text-gray-400"} />
              <span className={isActive ? "text-green-700 font-semibold" : "text-gray-500"}>{s.label}</span>
              {isActive && <motion.div layoutId="underline" className="h-1 w-6 bg-green-700 rounded-full mt-1"/>}
            </button>
          );
        })}
      </header>
            ...
            >
              {sec.id === "home" && (
                <>
                  <div className="max-w-3xl mx-auto text-center space-y-6">
                    ...
                    <p className="text-lg">Alătură-te flotei noastre autorizate și începe să câștigi bani conducând pentru <strong>Bolt</strong> sau <strong>Uber</strong> în Cluj! Poți folosi <strong>propriul autoturism</strong> sau unul <strong>pus la dispoziție de noi</strong>. Oferim suport complet pentru a începe cât mai ușor și rapid.</p>
                    <p className="text-lg text-green-700">Inclusiv posibilitatea de a <strong>achizitiona autoturismul dorit prin chirie cu ramanere</strong>, lucrând ca șofer în cadrul flotei noastre. Totul transparent, documentat legal și adaptat nevoilor tale.</p>
                    ...
                    <button onClick={() => scrollTo('contact')} className="mt-8 bg-green-700 hover:bg-green-800 text-white font-semibold px-8 py-3 rounded-full shadow-lg transition">
                      Aplică acum
                    </button>
                  </div>
                </>
                ...
                <div className="max-w-3xl mx-auto space-y-6">
                  ...
                  <ul className="list-none text-lg space-y-3">
                    {[
                      ...
                      <li key={i}><Check className="inline-block w-5 h-5 text-green-700 mr-2"/> {item}</li>
                    ))}
                  </ul>
                </div>
              )}
              {sec.id === "cerinte" && (
                <div className="max-w-3xl mx-auto space-y-6">
                  ...
                  <p className="text-lg">Serviciile de transport alternativ sunt reglementate de OUG 49/2019. Pentru a desfășura această activitate trebuie să îndeplinești:</p>
                  <ul className="list-decimal list-inside text-lg space-y-3">
                    ...
                    <li>Cazier auto cu istoricul pe ultimii 5 ani (fără suspendarea dreptului de a conduce în ultimul an pentru conducerea sub influența băuturilor alcoolice sau a substanțelor psihoactive sau implicarea în accidente rutiere grave)</li>
                    <li>Adeverință de la medicul de familie care să ateste că ești clinic sănătos</li>
                  </ul>
                </div>
              )}
              {sec.id === "masini" && (
                <div className="max-w-4xl mx-auto space-y-6">
                  ...
                  {masini.map(car => (
                    <motion.div key={car.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="bg-white rounded-xl shadow-md p-6 mb-6">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-6 flex-wrap">
                        <div className="flex flex-col flex-shrink-0">
                          <img src={car.images[0]} alt={car.title} className="w-full sm:w-64 h-40 object-cover rounded mb-4 cursor-pointer" onClick={()=>openImageViewer(car, car.images[0])}/>
                          <div className="flex gap-2">
                            {car.images.slice(1,4).map((img,i)=>(<img key={i} src={img} className="h-16 w-20 object-cover rounded cursor-pointer" onClick={()=>openImageViewer(car,img)}/>))}
                          </div>
                        </div>
              ...
              {sec.id === "contact" && (
                <div className="max-w-3xl mx-auto space-y-6 text-center">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-6 text-sm">
                    <div className="flex items-center gap-2"><Building2 className="w-5 h-5"/><div><p>SC Flota Trans SRL</p><p>CUI: RO12345678</p></div></div>

                    <div className="flex items-center gap-2">
                    <Phone className="w-5 h-5" />
                    <FaWhatsapp size={20} className="text-green-500" />
                    <a href="tel:+40712345678" className="underline">+40 712 345 678</a>
                    </div>

                    <div className="flex items-center gap-2"><Mail className="w-5 h-5"/><a href="mailto:flota@example.com" className="underline">flota@example.com</a></div>
                  </div>
                </div>
              )}
            </motion.section>
      ...
      <AnimatePresence>
        {selectedCar && (
          <motion.div className="fixed inset-0 bg-black bg-opacity-80 flex flex-col items-center justify-center z-50" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <button className="absolute top-6 right-6 text-white" onClick={closeImageViewer}><X size={32}/></button>
            <img src={selectedImage} className="max-w-full max-h-[80vh] mb-4 object-contain rounded-lg"/>
            <div className="flex gap-3 overflow-x-auto px-4">
              {selectedCar.images.map((img,i)=>(<img key={i} src={img} className={`h-20 w-24 object-cover rounded cursor-pointer ${selectedImage===img?"border-2 border-white":""}`} onClick={()=>setSelectedImage(img)}/>))}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
